...
- **依赖模块**:
  - 依赖服务: mall-user
  - 依赖数据库表: aim_employee（本需求同步设计）, aim_job_type（查询归档）, aim_agent_product（查询归档）
...
- **验收标准**:
  ...
  - [ ] 校验商品代理冲突
  - [ ] 调用mall-user查询用户信息
- **归档内容**: 完成后归档表结构到 `feature-archive.md#aim_employee`
...
- **依赖模块**:
  - 依赖服务: mall-user
  - 依赖数据库表: aim_quota_config（本需求同步设计）, aim_employee（依赖REQ-031归档）
...
- **前置查询**: 实现前查询 `feature-archive.md#aim_employee` 获取员工表结构
...
- **依赖模块**:
  - 依赖服务: mall-user
  - 依赖数据库表: aim_activation_record（本需求同步设计）, aim_employee（查询归档）
...
- **依赖模块**:
  - 依赖数据库表: aim_employee_script（本需求同步设计）
- **被依赖**: REQ-008, REQ-012, REQ-028, REQ-029
- **验收标准**:
  - [ ] 设计 aim_employee_script 表结构
  - [ ] 支持话术CRUD
  - [ ] 提交后状态为"待审核"
  - [ ] 审核通过后才生效
  - [ ] 话术匹配（相似度≥0.85）
- **归档内容**: 完成后归档表结构到 `feature-archive.md#aim_employee_script`
...
- **依赖模块**:
  - 依赖数据库表: aim_employee, aim_style_config, aim_job_type（均查询归档）
...
- **依赖模块**:
  - 依赖数据库表: aim_employee, aim_conversation, aim_employee_script, aim_knowledge_file（均查询归档）
- **被依赖**: REQ-017
- **验收标准**:
  - [ ] 统计员工总数、上线数
  - [ ] 统计今日咨询量
...
| 7 | aim_employee_script | mall-agent | 话术表 | REQ-034 | feature-archive.md#aim_employee_script |
| 8 | aim_knowledge_file | mall-agent | 知识库文件表 | REQ-035 | feature-archive.md#aim_knowledge_file |
| 9 | aim_employee | mall-agent | 智能员工表 | REQ-031 | feature-archive.md#aim_employee |
...
| REQ-031 | mall-user | aim_employee（本需求设计）, aim_job_type（归档）, aim_agent_product（归档） | REQ-001~006, 011, 017, 023, 024 |
| REQ-032 | mall-user | aim_quota_config（本需求设计）, aim_employee（归档） | REQ-001, 002, 019, 031 |
| REQ-033 | mall-user | aim_activation_record（本需求设计）, aim_employee（归档） | REQ-004, 015, 016, 022 |
| REQ-034 | - | aim_employee_script（本需求设计） | REQ-008, 012, 028, 029 |
...
7. **REQ-034**: 话术管理服务（设计 aim_employee_script 表）→ 归档
8. **REQ-035**: 知识库管理服务（设计 aim_knowledge_file 表）→ 归档
9. **REQ-031**: 智能员工应用服务（设计 aim_employee 表，查询 aim_job_type/aim_agent_product 归档）→ 归档
10. **REQ-032**: 名额管理服务（设计 aim_quota_config 表，查询 aim_employee 归档）→ 归档
11. **REQ-033**: 解锁与激活服务（设计 aim_activation_record 表，查询 aim_employee 归档）→ 归档
...
14. **REQ-037**: Prompt生成服务（查询 aim_employee/aim_style_config/aim_job_type 归档）
...
| aim_employee_script | REQ-034 | feature-archive.md#aim_employee_script | REQ-008, REQ-012, REQ-028 |
| aim_knowledge_file | REQ-035 | feature-archive.md#aim_knowledge_file | REQ-009, REQ-012, REQ-030 |
| aim_employee | REQ-031 | feature-archive.md#aim_employee | REQ-001~007, REQ-011, REQ-017, REQ-023~025, REQ-032, REQ-033, REQ-036, REQ-037, REQ-043 |
