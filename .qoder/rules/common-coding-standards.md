...
description: Java 编码规范 - 通用规范，适用于任何 Java 项目，定义代码质量、命名、异常处理等标准
---
...
|----------|------------|-------------|------|
| **单表查询** | `XxxDO` | `CommonResult<CommonResult.PageData<XxxResponse>>` | Service层完成DO到Response转换 |
...
| 场景 | 调用方 | 实现方 | 说明 |
|------|--------|--------|------|
| **增删改** | `XxxManageService` | `XxxService` (MP) | 使用 MP IService 提供的 `save()`/`updateById()`/`removeById()` |
| **小表分页** (< 100万) | `XxxQueryService` | `XxxService` (MP) | 使用 MP `page()` 方法 |
| **大表分页** (>= 100万) | `XxxQueryService` | `XxxService` → `XxxMapper` | 封装索引覆盖分页逻辑 |
| **非分页查询** | `XxxQueryService` | `XxxService` → `XxxMapper` | 封装原生 MyBatis XML 查询 |

**分层调用原则**：

- **XxxQueryService / XxxManageService** 只能调用 `XxxService`
- **XxxService** 封装所有数据访问，包括 MP 方法和原生 Mapper 调用
- **XxxMapper** 只对 `XxxService` 暴露，其他层禁止直接调用
...
| 类型 | 命名规则 | 示例 | 所属模块 | 说明 |
|------|----------|------|----------|------|
| 实体类 | 大驼峰 + DO 后缀 | `{Prefix}AgentDO`、`{Prefix}UserDO` | 应用服务 | 数据库实体，统一前缀 |
...
| Service | 前缀 + 大驼峰 + Service 后缀 | `{Prefix}AgentService` | 应用服务 | MP数据服务，继承IService |
| Mapper | 前缀 + 大驼峰 + Mapper 后缀 | `{Prefix}AgentMapper` | 应用服务 | 数据访问层 |
