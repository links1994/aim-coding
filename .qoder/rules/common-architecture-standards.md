...
| **Manage**      | 管理服务 | `XxxManageService`      | 增删改操作，封装写逻辑                       | **只能调用 AimXxxService**，禁止直接调用 XxxMapper |
| **Data**        | 数据服务 | `AimXxxService`         | 继承 MyBatis-Plus IService，封装所有数据访问 | 可调用 XxxMapper 进行原生查询                    |
...
**核心规范**：上层服务（QueryService/ManageService）**禁止**直接使用 `AimXxxService.getBaseMapper()` 方法访问
Mapper，必须在 `AimXxxService` 中封装方法提供间接访问。
