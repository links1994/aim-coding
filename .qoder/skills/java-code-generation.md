...
2. **知识库查询（自动复用已有代码）**
   ...
   **使用 knowledge-base-query Skill 查询**：
   ...
   目的:
   - 查询类似功能的代码结构
   - 复用已有的 Controller/Service/Mapper 模式
   - 参考已有的 DTO/VO 定义
   ```
   ...
   **查询类型及用途：**
   ...
   | 查询类型 | 用途 | 复用内容 |
   |----------|------|----------|
   | feature | 类似功能档案 | Controller/Service/Mapper 代码结构 |
   | schema | 数据库表结构 | 现有字段定义，避免命名冲突 |
   | api | 内部/外部API | Feign接口定义、第三方API调用模式 |
   | pitfall | 历史坑点 | 避免已知陷阱（如Feign客户端重复创建） |

   **查询结果处理**：
   - 如有相似功能的代码 → 参考其代码结构，复用可复用的代码片段
   - 如无相似功能 → 按规范生成新代码
